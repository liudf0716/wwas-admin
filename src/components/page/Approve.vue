    ...
    export default {
        data: function () {
            return {
                ...
                forms: { // Encapsulates all form data models, organized by feature.
                    wxpay: { // WeChat Pay settings form model
                        mch_id: '',
                        mch_serial_no: '',
                        mch_v3_key: '',
                    ...
                    onekey: { // One-key authentication settings form model
                        enable: true,
                        from_server: true, // Indicates if one-key auth is server-side managed
                    },
                    user: { // User password authentication settings form model
                        username: 'apfree',
                        password: 'apfree',
                        enable: true,
                        from_server: true, // Indicates if user auth is server-side managed
                    },
                    base: { // Base settings form model (portal URL, session duration, client timeout, background image)
                        portal_url: '',
                        duration: '',
                        client_timeout: '',
                    ...
                    ali: { // AliYun SMS provider settings form model
                        app_id: '',
                        app_secret: '',
                        sms_sign_name: '',
                        sms_template_code: '',
                    ...
                    wy: { // WangYi (NetEase) SMS provider settings form model
                        app_id: '',
                        app_secret: '',
                        template_id: '',
                        enable: false,
                    ...
                    none: { // Model representing the state when no SMS provider is chosen (disabling SMS auth)
                        enable: false,
                    }
                },
                rules: { // Encapsulates all form validation rules, mirroring the 'forms' object structure.
                    wxpay: { // Validation rules for WeChat Pay settings
                        mch_id: [
                    ...
                    user: { // Validation rules for User Password Authentication settings
                        username: [
                    ...
                    base: { // Validation rules for Base Settings
                        portal_url: [
                    ...
                    ali: { // Validation rules for AliYun SMS provider settings
                        app_id: [
                    ...
                    wy: { // Validation rules for WangYi (NetEase) SMS provider settings
                        app_id: [
                ...
                upload: { // Manages file upload configurations and state for different upload types.
                    bgImage: { // Configuration for background image upload
                    ...
                    wxpayKey: { // Configuration for WeChat Pay private key file upload
                        url: baseUrl + '/setting/uploadKey', // API endpoint for key upload
                        fileList: [] // Stores the file list for el-upload
                    },
                    wxpayCert: { // Configuration for WeChat Pay certificate file upload
        ...
        methods: {
            ...
            _buildSmsParams: function() {
                const self = this;
                if (self.dxchoose == 'ali') { // AliYun SMS
                    return {
                ...
                } else if (self.dxchoose == 'wy') { // WangYi SMS
                    return {
                ...
                } else { // No SMS provider selected or 'none'
                    return {
                        smsAliEnable: false,
                        smsWyEnable: false,
            ...
            onSaveSubmit: function(form_name){
                ...
                self.$refs[form_name].validate(function (valid) {
                    ...
                    self.$axios.post(baseUrl + '/setting/saveSetting',self.params).then(function (res) {
                        ...
                        if(res.data.ret_code == '1001'){ // Specific error code for session/auth issue
                            self.$message({message:res.data.extra,type:'warning'});
                            setTimeout(function(){
                                self.$router.replace('login'); // Redirect to login page
                            },2000)
                        }
                        if(res.data.ret_code == 0){ // Success
                            self.$message({message:res.data.extra,type:'success'});

                        }else{ // Other errors reported by the server
            ...
            getData: function(){
                ...
                self.$axios.post(baseUrl+'/setting/getSetting',params).then(function(res){
                    if(res.data.ret_code == '1001'){ // Session/auth issue
                        self.$message({message:res.data.extra,type:'warning'});
                        setTimeout(function(){
                            self.$router.replace('login'); // Redirect to login page
                        },2000)
            ...
            uploadSuccess: function(response, file, fileList) {
                if (response.ret_code == 0) { // Upload was successful on the server
                ...
                } else { // Server returned an error for the upload
...
<style scoped>
    ...
    .diainp{width:260px;} /* Standard input width */
    .diainp2{width:260px;} /* Another standard input width, if different styling needed */
    .inp100{width:100px;} /* Shorter input width */
    .form-box2{width:700px;} /* Width constraint for form boxes */
