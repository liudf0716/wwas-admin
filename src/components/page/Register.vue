    ...
    <div class="ms-login">
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="80px" class="demo-ruleForm">
        ...
        <el-form-item label="所在城市" prop="city" style="display: none">
          <el-select size="small" style="width: 110px" v-model="ruleForm.selectProv" placeholder="请选择省份" v-on:change="getProv($event)">
            <el-option v-for="item in provs" :key="item.label" :label="item.label" :value="item.value"> </el-option>
          </el-select>
          <el-select size="small" style="width: 104px" v-if="ruleForm.selectProv != ''" v-model="ruleForm.selectCity" placeholder="请选择城市" v-on:change="getCity($event)">
    ...
    submitForm: function (formName) {
      ...
      self.$refs[formName].validate(function (valid) {
        if (valid) {
          ...
          //console.log(params);
          self.$axios.post(global_.baseUrl + '/admin/register', params).then(
            function (res) {
              //console.log(res);
